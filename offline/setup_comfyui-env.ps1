...
$serviceDir = Join-Path $resourceDir "ComfyUI"
$envDir = Join-Path $resourceDir "comfyui-backend-env"

$offlineDir = $scriptDir
$wheelsDir = Join-Path $offlineDir "wheels\comfyui"
...
function Copy-ComfyUI {
    $sourceDir = Join-Path $offlineDir "ComfyUI"
    $destDir = Join-Path $resourceDir "ComfyUI"

    if (Test-Path $destDir) {
        Remove-Item -Recurse -Force $destDir
    }
    Write-Host "Copy ComfyUI from $sourceDir to $destDir"
    Copy-Item -Recurse -Force $sourceDir $destDir
    Write-Host "Copy ComfyUI [done]"

    # patch extra_model_paths.yaml file
    $pthPath = Join-Path $destDir "extra_model_paths.yaml"
    $pthContent = "
aipg:
  base_path: $resourceDir\service\models\stable_diffusion
...
"
    Set-Content -Path $pthPath -Value $pthContent
    Write-Host "Patched extra_model_paths.yaml file at $pthPath"

    # update workflow directory
    $workflowDir = Join-Path $resourceDir "workflows"
    if (Test-Path $workflowDir) {
        Remove-Item -Recurse -Force $workflowDir
    }
    Write-Host "Copy workflow from $offlineDir\workflows to $workflowDir"
    Copy-Item -Recurse -Force (Join-Path $offlineDir "workflows") $workflowDir
    Write-Host "Copy workflow [done]"
}
...
function Install-EnvRequirements {
    ...
    $requirementsPath = Join-Path $resourceDir "service\requirements-xpu.txt"
    $aiwheelsDir = Join-Path $offlineDir "wheels\ai-playground"
    Write-Host "Install requirements from $requirementsPath"
    Start-Process -FilePath $pythonPath -ArgumentList "-m uv pip install -r `"$requirementsPath`" --no-index --find-links `"$aiwheelsDir`"" -WorkingDirectory $envDir -NoNewWindow -Wait
