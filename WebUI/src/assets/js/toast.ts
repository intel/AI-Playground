 ...
 */
export function show(text: string, options?: ToastOptions, transitions?: ToastTransitions) {
  if (_toastStage != null) {
  ...
  const _options = mergeOptions(DEFAULT_SETTINGS, options || {}) as ToastOptions
...
export function warning(text: string, settings?: ToastSettings, mounted?: HTMLElement | string) {
  show(text, {
    mounted,
    style: {
 ...
 */
function showToast(text: string, options: ToastOptions, transitions: ToastTransitions) {
  ...
  if (options.settings && options.settings.duration !== 0) {
    _timeout = window.setTimeout(() => hideToast(transitions), options.settings.duration)
  }
}
