...
const currentImage = computed<MediaItem | null>(() => {
  return imageGeneration.generatedImages.find(
    (image) => image.id === imageGeneration.selectedVideoId
  ) ?? null
})
...
onMounted(() => {
  promptStore.registerSubmitCallback('video', generateImage)
  promptStore.registerCancelCallback('video', stopGeneration)
})

onUnmounted(() => {
  promptStore.unregisterSubmitCallback('video')
  promptStore.unregisterCancelCallback('video')
})

watch(
  () => imageGeneration.generatedImages.filter((i) => i.state !== 'queued').length,
  () => {
    const nonQueuedImages = imageGeneration.generatedImages.filter((i) => i.state !== 'queued' && i.mode === 'video')
    if (nonQueuedImages.length > 0) {
      imageGeneration.selectedVideoId = nonQueuedImages[nonQueuedImages.length - 1].id
    } else {
      imageGeneration.selectedVideoId = null
    }
...
async function generateImage(prompt: string) {
  ...
  await imageGeneration.generate('video')
}
