<template>
  <div
    class="absolute left-0 top-0 cursor-none"
    :style="{ width: `${props.width}px`, height: `${props.height}px` }"
    @mouseenter="showShadow"
    @mousemove="moveWithShadow"
    @mouseleave="shadowPos.show = false"
  >
    <canvas
      :width="props.width"
      :height="props.height"
    ...
    ></canvas>
    <span
      v-show="shadowPos.show"
      class="absolute bg-gray-500 pointer-events-none opacity-50 rounded-full"
      :style="{
        width: `${props.brushSize}px`,
        height: `${props.brushSize}px`,
        transform: shadowPos.translate,
      }"
    ></span>
  </div>
</template>
