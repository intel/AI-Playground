...
function getPercentRealVal(per: number): number {
    ...
    while (left < props.max) {
        ...
        if (left == testVal) {
            ...
            return Math.abs(testVal - right) < Math.abs(testVal - left)
                ? right : left;
        }
        left = right;
    }
...
<template>
    <div class="v-vertical-slide" :class="{ 'disabled': disabled }">
        <div class="v-slide-container">
            <slot name="min" v-if="showRange"><span class="v-slide-min">{{ props.min }}</span></slot>
            ...
            <slot name="max" v-if="showRange"><span class="v-slide-max">{{ props.max }}</span></slot>
        </div>
        <input class="v-slide-val" v-if="showEditbox" :value="props.current" type="text" @input="inputChange"
            @change="changeLimit" />
    </div>
</template>