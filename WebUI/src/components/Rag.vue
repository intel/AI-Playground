<template>
    <div class="rag-panel flex flex-col text-white">
        <div class="rag-header flex justify-between items-center h-11 px-4 border-b border-gray-200 text-sm">
            <span>{{ fileTotalText }}</span>
            <div class="flex items-center gap-2">
                <button class="svg-icon i-add-box w-5 h-5 text-white hover:text-purple-500" @click="chooseUploadFiles">
                </button>
                <button class="svg-icon i-close w-5 h-5 hover:text-purple-500" @click="closeRagPanel">
                </button>
            </div>
        </div>
        <div class="flex-auto overflow-y-auto px-4 py-2" v-show="fileTotal > 0" @drop="dropFileToUpload"
            ...
            <div v-for="item in fileList" class="flex items-center gap-2 justify-center w-full h-8">
                ...
                <div class="flex-grow line2 overflow-hidden text-ellipsis h-6" :title="item.filename">{{
                    item.filename }}</div>
                <span class="svg-icon i-queue flex-none w-5 h-5" v-if="item.status == 0">
                </span>
                <span class="svg-icon i-loading flex-none w-5 h-5" v-else-if="item.status == 1">
                </span>
                <button class="svg-icon i-delete flex-none w-5 h-5 hover:text-red-500" v-if="item.status == 2"
                    :disabled="opLocker" @click="deleteFile(item)">
                </button>
            </div>
        </div>
        <div v-show="fileTotal == 0"
            class="flex-auto h-0 flex flex-col items-center gap-2 justify-center text-gray-400 select-none"
            @dragover="dragOverHandler" @drop="dropFileToUpload">
            <p class="text-lg font-bold" v-if="!globalSetup.state.isAdminExec">{{ i18nState.RAG_DRAG_UPLOAD }}</p>
            <pre class="text-xs" v-if="!globalSetup.state.isAdminExec">{{ i18nState.RAG_UPLOAD_MIME_TYPE }}</pre>
...
function getIconClass(type: number) {
    switch (type) {
        case 2: return "i-word";
        case 3: return "i-ppt";
        case 4: return "i-pdf";
        case 5: return "i-md";
        default: return "i-txt";
    }
}
...
async function getIndexFiles() {
    ...
    const rspJson = await response.json() as ApiResponse & { data: { filename: string, md5: string }[] };
    ...
    for (let i = 0; i < filePaths.length; i++) {
        ...
        if (idx > -1) {
            ...
            if (!/\.(docx?|txt|pdf|md)$/i.test(ext)) {
                ...
                fileList.value.push({ type: getFileType(ext), filename: filePath.substring(filePath.lastIndexOf('\\') + 1), md5: "", status: 0, path: filePath });
                successCount++;
            }
        }
    }
    ...
    while (true) {
        ...
        if (fileItem.path) {
            try {
                ...
                const response = await fetch(`${globalSetup.apiHost}/api/llm/uploadRagFile`, { method: "post", body: formData });
...
async function deleteFile(index: RagFileItem) {
    if (opLocker.value) { return; }
    try {
        opLocker.value = true;
        const formData = new FormData();
        formData.append("md5", index.md5);
        const response = await fetch(`${globalSetup.apiHost}/api/llm/deleteRagIndex`, { method: "POST", body: formData });
